...
function redirectToGoogleSearch(picker) {
  ...
  if ('geolocation' in navigator) {
    ...
    const timeout = setTimeout(() => window.location = `http://google.com/search?q=` + picker.result, 11000)
    console.log('timeout set');
    navigator.geolocation.getCurrentPosition( position => {
      console.log('inside success callback for position');
      clearTimeout(timeout);
      console.log('timeout cleared');
      const geocoder = new google.maps.Geocoder();
      const latlng = new google.maps.LatLng(position.coords.latitude, position.coords.longitude);
      geocoder.geocode({'latLng': latlng}, function(results, status) {
        console.log('trying to get city');
    ...
    }, () => window.location = `http://google.com/search?q=` + 'rejected')
  } else {
